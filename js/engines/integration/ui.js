import { getAllIntegrations, getIntegrationStatus, getIntegrationConfig, getIntegrationFullConfig, configureIntegration, disconnectIntegration } from './index.js';
...
function _renderWithCopyButtons(text) {
  return text.replace(/(https?:\/\/[^\s<>"')}\]]+)/g, (url) => {
    return `<span class="int-url-copy-wrap"><code class="int-url-code">${url}</code><button class="int-copy-url-btn" data-url="${url}">Copiar</button></span>`;
  });
}
...
async function _showConfigForm(id) {
  ...
  body.innerHTML = `<div class="int-cfg-header">
    ...
    <div class="int-cfg-desc">${_renderWithCopyButtons(int.description)}</div>
  ...
  body.querySelectorAll('.int-copy-url-btn').forEach(b => {
  ...
  body.querySelector('.guide-btn')?.addEventListener('click', () => {
    ...
    <div class="int-guide-steps">${steps.map(s => `<div class="int-guide-step"><span class="int-gs-num">${s.match(/^(\d+)/)?.[1] || '○'}</span><span class="int-gs-text">${_renderWithCopyButtons(s.replace(/^\d+\.\s*/, ''))}</span></div>`).join('')}</div>
