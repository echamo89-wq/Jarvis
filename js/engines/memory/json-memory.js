...
const _log = createLogger('MEMORY-JSON');
...
export class JsonMemory {
  ...
  async load() {
    try {
      ...
      if (raw) {
        ...
        _log('info', 'Memoria cognitiva JSON cargada desde disco');
      }
      return this._stores;
    } catch (e) {
      _log('warn', `Memoria JSON load: ${e.message}`);
      return this._stores;
    }
  }
  ...
  async persist() {
    try {
      ...
      _log('error', `Persist JSON: ${e.message}`);
    }
  }
